...
def first_frame_lane_finder(bird_eye_binary, M, img, nwindows = 9, debug=False):
    ...
    -------
    If debug == True:
        return image with left lane pixels (red) and right lane pixels (blue) 
        in flat bird-eye view.
    Else:
        return original image with left lane pixels (red), right lane pixels (blue) 
