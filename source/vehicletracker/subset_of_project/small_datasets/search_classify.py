...
def efficient_find_cars(img, y_start, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins, debug=False):
    ...
    cells_per_step = 2 # Instead of overlap, define how may cells to step
    ...
    if debug == False:
        ...
        for xb in range(nxsteps):
            for yb in range(nysteps):
                ...
                # print("y_start:", y_start+ytop_draw, "y_end:", ytop_draw+win_draw+ystart)
...
ystop = 707
scale = 1.5
# out_img = efficient_find_cars(image, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins, debug=False)


out_img, box_list = efficient_find_cars(image, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins, debug=False)
plt.imshow(out_img)
plt.show()

# Save the data after preprocessing step for easy access
pickle_file = 'bbox_pickle.p'
# if not os.path.isfile(pickle_file):
print('Saving data to pickle file...')
try:
    with open(pickle_file, 'wb') as pfile:
        pickle.dump(box_list,
...
# Check the prediction time for several samples
# t = time.time()
# n_predict = 10
